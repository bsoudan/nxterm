...
func TestTermdHandleRequest(t *testing.T) {
	...
	runner := layer.NewTaskRunner[RenderState]()

	var got any
	var gotErr error
	done := make(chan struct{})

	runner.Run(func(h *layer.Handle[RenderState]) {
	...
	result := cmd()
	tsm, ok := result.(layer.TaskSendMsg)
	if !ok {
...
func TestTermdHandleRequestCancelled(t *testing.T) {
	...
	id := runner.Run(func(h *layer.Handle[RenderState]) {
		th := &TermdHandle{Handle: h}
