...
// ErrProgramQuit is returned by [Program.Handle] when a [QuitMsg] is received.
var ErrProgramQuit = errors.New("program quit")
...
type Program struct {
	...
	// model is the current model, moved from local var in Run to support
	// the Start/Handle/Stop split.
	model Model

	// cmds is the command runner channel, moved from local var in Run.
	cmds chan Cmd
}
...
// Handle processes a single message through bubbletea's event loop body:
// translate input, filter, pre-process internals, Update model, run cmd, render.
// Returns the processed message (nil if filtered or internal-only).
// Returns ErrProgramQuit for QuitMsg, ErrInterrupted for InterruptMsg.
func (p *Program) Handle(msg Msg) (Msg, error) {
	...
	select {
	case <-p.ctx.Done():
		return msg, ErrProgramQuit
	case p.cmds <- cmd: // process command (if any)
	}

	p.Render() // render view
	return msg, nil
}
...
// Render calls model.View() and sends the result to the renderer.
// Used after the main loop processes non-bubbletea messages directly.
...
// Start does everything Run() does before eventLoop():
// terminal init, renderer, input reader, Init(), initial render.
// After Start returns, call Msgs() to get the message channel and
// Handle() to process messages in your own select{}.
func (p *Program) Start() error {
	if p.initialModel == nil {
		return errors.New("bubbletea: InitialModel cannot be nil")
	}
	...
	if p.ttyOutput != nil {
		...
		if err != nil {
			return fmt.Errorf("bubbletea: error getting terminal size: %w", err)
		}
		width, height = w, h
	}
	...
	// Process commands.
	p.handlers.add(p.handleCommands(p.cmds))

	return nil
}

// Msgs returns the message channel. The caller reads from this
// in their own select{} and calls Handle() for each message.
func (p *Program) Msgs() <-chan Msg {
	return p.msgs
}

// Context returns the program's context (cancelled on Kill/signal).
func (p *Program) Context() context.Context {
	return p.ctx
}

// Stop does everything Run() does after eventLoop():
// final render, shutdown, restore terminal. Returns final model.
// The err parameter is the error from eventLoop (nil for graceful quit,
// ErrProgramQuit for QuitMsg, ErrInterrupted for InterruptMsg).
func (p *Program) Stop(loopErr error) (Model, error) {
	defer close(p.finished)
	defer p.cancel()

	err := loopErr
	// Normalize ErrProgramQuit to nil — it's a clean exit.
	if errors.Is(err, ErrProgramQuit) {
		err = nil
	}
